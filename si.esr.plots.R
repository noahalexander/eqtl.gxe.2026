...
ggsave("lod_plot.byxrm.png", p, width = 14, height = 7, dpi = 300, bg = "white")


##################3----------BYxRM sp esr activity plots
...
p <- ggplot(plot_df, aes(x = pos, y = LOD, color = state, linetype = time)) +
  ...
  scale_color_manual(values = state_colors, name = NULL) +
  scale_linetype_manual(values = c("t0" = "solid", "t10" = "dashed"), name = NULL) +
  scale_x_continuous(breaks = breaks_all, labels = labels_all, expand = c(0, 0)) +
  scale_y_continuous(limits = c(0, y_cap), expand = c(0, 0), oob = squish) +
  labs(x = NULL, y = "LOD") +
  theme_classic(base_size = 16) +
...
# ---------- Load CBSxYJM ----------
A_t0 <- prep_lod_long(
  "/Users/noahalexander/repeat_fine_mapping/combined/3004/nacl/t0/state_pheno_LODs.RDS",
  "t0"
)

A_t30 <- prep_lod_long(
  "/Users/noahalexander/repeat_fine_mapping/combined/3004/nacl/t30/state_pheno_LODs.RDS",
  "t30"
)
...
p
ggsave("lod_plot.3004.png", p, width = 14, height = 7, dpi = 300, bg = "white")
